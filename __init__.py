...
class Ledger:
    ...
    def create_account(self, name):
        ...
        with self.connection.begin():
            ...
            sql = text('SELECT account_id FROM accounts WHERE name=:name FOR UPDATE')
            sql = sql.bindparams(name=name)
            logging.debug(sql)
            results = self.connection.execute(sql)
    ...
    def _transfer_funds(self, debit_account_id, credit_account_id, amount):
          # sql = f'LOCK TABLE balances IN ACCESS EXCLUSIVE MODE'
          # logging.debug(sql)
          # self.connection.execute(sql)
        try:
            with self.connection.begin():
            ...
            logging.error(f"Error transferring funds: {e}")
            if self.attempt < self.max_attempts:
                logging.info(f"Retrying transfer (Attempt {self.attempt})")
                self.attempt += 1
                self._transfer_funds(debit_account_id, credit_account_id, amount)
            else:
                logging.error("Maximum attempts reached. Transfer failed.")
